...
export class DatabaseUtil {
     ...
     */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    public getRepository<T extends object>(entity: new (...args: any[]) => T): Repository<T> {
        try {
            ...
            if (DatabaseUtil.connection) {
                ...
                // Use 'as' casting here so TypeScript knows this specific array slot matches Repository<T>
                return this.repositories[entityName] as Repository<T>;
            }
            
            // If connection isn't initialized yet, throw an error to prevent returning null/undefined
            ...
            throw error; // Re-throw the error so your services know initialization failed
        }
    }
